...
object RetweetResult extends PipelineResult[RetweetResultJson] {
  override def addToStream(stream: DataStream[Tweet], sinkFunction: SinkFunction[RetweetResultJson], props:TweetStreamProps): Unit = {
    stream
      .filter(_.retweet)
  ...
  override def addToStream(stream: DataStream[Tweet], props:TweetStreamProps): Unit = {
    addToStream(stream, ElasticUtils.createSink[RetweetResultJson]("retweets-idx","retweets-timeline", props.elasticUrl), props)
  }
